...
- **2026-09-25 (2)** — **Leave app Phase 3 + real SLA escalation.** The engine now honors the SLA promise every template makes: waiting approvals with `sla_hours` time out (fractional hours like "0.02" supported for fast demos), the breached gate is skipped with an audit note, and escalation steps (`condition: previous_step.sla_breached`) stop being decorative — they wait on the escalation approver (`TickAllAt` gives tests a deterministic clock; SLA-01..05 green, full suite 17 packages). Leave workflow v2 adds the escalation step; the app surfaces it: "escalated 🔥" pill, employee notification ("manager did not respond within the SLA"), HR holds the card. Phase-3 polish also shipped: per-request **workflow timeline** (audit cross-link via `/api/leave/{id}/steps`), unread notification counts with mark-on-view, and pending-days in the balance pill. Live-verified end-to-end: LV-005 breached at 0.02h → escalated → HR approved → balance validated → final gate → callback → deducted 13→11. Fixed a service crash found during verification (old `data.json` lacked the new `readAt` field — state now merges seed defaults on load).

- **2026-09-25** — **Leave app Phase 2: the tiny React app is live.** `leave-app/server.mjs` (zero-dep Node): serves the Vite-built page, owns balances/org-map/history (persisted to `data.json`), submits through the FlowForge webhook, live-polls instances, exposes the unchanged workflow-facing routes (balance + callback) — it replaced the Phase-1 stub on :9090 with zero connector changes. `leave-app/web/` (React 19 + Vite, no UI deps): identity picker, Apply form with auto day-count, My Requests with live status pills + notification bell/badge, and the Approvals inbox routed by org map (manager gate) and HR role (final gate) with ✓/✕ actions. All three verdict paths verified live through the app's own API: approved (LV-003: manager → HR → callback → deducted 18→13 + notifications), approver-rejected (LV-004: cancelled, balance untouched, notification), and insufficient balance (submit-time pre-check rejects 5d vs 2d cleanly; the connector 409 remains the in-flight safety net).

- **2026-09-24** — **Leave app Phase 1: the loop is proven.** First downstream application on FlowForge: a standalone leave-request app that delegates its process entirely to a workflow. Shipped: `leave-request.flow.yaml` (validate → manager approval → **live balance validation connector** → HR final approval → **verdict callback connector**), two drop-in connectors (`balance-check` GETs the leave app's authoritative balance incl. `?days=`; `leave-callback` POSTs the verdict with a shared-secret header), and a zero-dep Node stub service (balance store + callback receiver + deduction-on-approval). Verified live end-to-end on the local server: approved path — webhook submit → both gates → callback received → balance deducted 18→13; insufficient-balance path — connector 409 → run failed, no callback, no deduction. The integration caught a real design gap (balance endpoint needed the requested days) and fixed it. Verdict semantics documented (completed/cancelled/failed → app meanings). `scripts/run-local.ps1` now loads the connectors drop-in dir by default.
...
- **2026-08-29 (2)** — **v0.1.0 RELEASED** 🎉 https://github.com/santhoshmp/flowforge/releases/tag/v0.1.0 — 24 assets: 6 platform archives (linux/darwin/win × amd64/arm64), SHA256SUMS, SPDX SBOM, and cosign keyless `.sig`+`.pem` for every artifact; multi-arch image on `ghcr.io/santhoshmp/flowforge:v0.1.0` (digest-signed). Getting there hardened the pipeline: the first v0.1.0 run silently lost the two darwin `.sig` uploads → added fail-fast signing (output must be non-empty) + a 24-artifact completeness gate; tag delete+re-push converts a release to a DRAFT → the release step now publishes/repairs (`gh release create`/`upload --clobber` + `edit --draft=false --latest`). RC1/RC2 releases remain as historical release candidates (delete in the UI if undesired).

- **2026-08-29** — **Release pipeline proven end-to-end; v0.1.0 cut.** RC1 verified the full binary surface (24 assets: 6 platform archives, SHA256SUMS, SPDX SBOM, cosign `.sig`+`.pem` per artifact) but its multi-arch image job was cancelled after 20 min of QEMU emulation. Fix: Dockerfile now pins UI/Go stages to `--platform=$BUILDPLATFORM` and cross-compiles per `TARGETOS/TARGETARCH` (CGO off) — QEMU is no longer in the build path; release jobs gained `timeout-minutes`. RC2 then ran the ENTIRE pipeline green in ~3 minutes, including ghcr multi-arch push + digest + cosign keyless image signing. Chart `appVersion` now carries the `v` prefix to match published image tags. Release gates per `docs/release.md`: CI green ✓, RC verified ✓, appVersion aligned ✓, changelog current ✓.
