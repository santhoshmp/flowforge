...
| | |
|---|---|
| **Last updated** | 2026-08-22 (product-wide suite: STORE/SEC-deep/ENG-deep/CONN-auth/PLG-host/API-full/E2E) |
...
| F-EXEC-02 | Human-task wait + resume | `src/engine.ts` → `approveWaiting`; `routes.ts` `/approve` | ENG-01, ENG-02, API-04, API-15, E2E-07 | ✅ |
| F-EXEC-03 | Retry from failed step (no re-run of completed) | `src/engine.ts` → `retryFailed`; `routes.ts` `/retry` | ENG-04, ENG-08, API-16 | ✅ |
| F-EXEC-04 | Cancel + `endedAt` | `src/engine.ts` → `cancelInstance`; `routes.ts` `/cancel` | ENG-05, ENG-10, API-13 | ✅ |
| F-EXEC-05 | Condition eval + auto-approve from input | `src/engine.ts` (condition branch) | ENG-02, ENG-03, API-16 | ✅ |
| F-EXEC-06 | SLA-escalation skip | `src/engine.ts` (`previous_step.sla_breached`) | ENG-02, API-15 | ✅ |
...
| F-API-01 | REST surface (`/api/v1/*`) | `src/routes.ts` → `registerRoutes` | API-01..14, E2E-05..07 | ✅ |
| F-API-02 | Per-workflow executions + run-with-input | `routes.ts` (`/workflows/:id/executions`) | API-04, API-05, API-07, API-13 | ✅ |
...
| F-PERSIST-01 | SQLite (durable, restart-safe) | `src/db.ts` → `openDB`, mappers, upserts | all (via `:memory:`) + STORE-01..04 + E2E-07 restart | ✅ |
| F-PERSIST-02 | Postgres backend + migration framework | *planned* | — | ⬜ P5 |
| F-SEC-01 | Built-in auth (bcrypt + HMAC tokens) + first-run admin setup + setup-mode gating | `server-go/internal/auth`, `server-go/internal/store/users.go`, `server-go/internal/api/auth_test.go` | SEC-01, SEC-01b..e, E2E-05 | ✅ |
...
| F-SEC-05 | Sandboxed real execution: Starlark script sandbox + egress-gated HTTP (opt-in via `code`/`url`) | `server-go/internal/executor`, `server-go/internal/engine`, `server-go/internal/executor/executor_test.go` | SEC-03, ENG-06..09 | ✅ |
...
| F-EXT-02 | Connector SDK: manifests, embedded built-ins + drop-in dirs, params validation, `${params/input/secret/env}` templating, http/smtp/wasm executors, approve-gate validation, redacted dry-run test | `server-go/internal/connectors` (+`connectors_test.go`, `auth_modes_test.go`), `internal/api/ext.go`, CLI `connector validate/test` | CONN-01..07, EXT-03, E2E-03 | ✅ |
| F-EXT-03 | Encrypted secrets vault (AES-256-GCM at rest, names-only API, env-key support) | `server-go/internal/secrets` (+`secrets_test.go`), `internal/api/ext.go` | SEC-04, E2E-07 | ✅ |
| F-EXT-04 | WASM plugin ABI + runtime (`ff` host module) | `server-go/internal/wasm` | PLG-01..10 | ✅ |
| F-EXT-05 | Template gallery: six embedded `flowforge/v1` artifacts + instantiate → draft | `server-go/internal/templates` (+`templates_test.go`, `gallery_loop_test.go`), `internal/api/ext.go` | TPL-01..03, API-15, E2E-07 | ✅ |
...
| F-SLA-01 | Real SLA escalation: waiting approvals time out (fractional-hour SLAs supported), breach skips the gate with an audit note, escalation steps fire | `engine/tickWaiting` + resolve fall-through, `TickAllAt` deterministic clock | SLA-01..05 | ✅ |
