...
// A control registered in the admin registry — built-in or custom
export interface ControlDef {
  key: string; // step type id, e.g. "ai.extract" or "custom.send_sms"
  label: string;
  color: string;
  icon: string;
  enabled: boolean;
...
export interface WorkflowStep {
  id: string;
  type: string; // control key — built-in StepType or a custom control
  name: string;
  params: Record<string, string>;
  // AI authoring metadata (the trust layer)
  confidence: number; // 0-100
  assumptions: string[];
  edited?: boolean;
  // canvas position (designer only — not serialized to the DSL)
